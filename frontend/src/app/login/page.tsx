...
export default function LoginPage() {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      // Login gagal: baca response error dari backend.
      const fieldErrors: {
        email?: string;
        password?: string;
        general?: string;
      ...
      if (!fieldErrors.email && !fieldErrors.password && data.message) {
  ...
  return (
    <main className="relative flex h-dvh items-center justify-center overflow-hidden overscroll-none bg-linear-to-tr from-teal-800 via-teal-950 to-neutral-950 p-4 text-slate-900 sm:p-6 lg:p-8">
      ...
      <div className="relative flex max-h-full w-full max-w-4xl flex-col overflow-x-hidden overflow-y-auto rounded-2xl bg-white shadow-2xl shadow-black/30 lg:min-h-[520px] lg:flex-row">
        <aside className="relative z-10 hidden min-w-0 flex-col overflow-hidden rounded-2xl bg-neutral-950 p-8 text-white lg:-mr-6 lg:flex lg:w-[44%]">
          ...
          <div className="relative mt-auto pt-10">
            ...
            <p className="mt-2.5 text-sm leading-relaxed text-white/80">
              Every application and interview, right where you need it.
            </p>
          </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-center rounded-2xl bg-white px-6 py-6 sm:px-8 lg:py-8 lg:pl-12 lg:pr-10">
          <div className="mx-auto w-full max-w-sm">
            ...
            <p className="mx-auto mt-2 max-w-xs text-center text-sm leading-relaxed text-slate-600">
              Track your career journey and stay on top of every opportunity.
            </p>

            <form onSubmit={handleLogin} noValidate className="mt-5 space-y-4 lg:mt-6">
              {errors.general && (
                <div
                  role="alert"
              ...
              <div>
                ...
                />
                {errors.email && (
                  <p id="login-email-error" className="mt-1.5 text-sm leading-snug text-red-600">
                    {errors.email}
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                />
                {errors.password && (
                  <p id="login-password-error" className="mt-1.5 text-sm leading-snug text-red-600">
                    {errors.password}
                  </p>
                )}
              </div>
